...
export abstract class EDAConnector {
  ...
  abstract getStatus(): Promise<{ connected: boolean; licenseValid: boolean }>;

  protected validateTask(task: EDATask): void {
    if (!task.id) throw new Error("Task ID is required");
    if (!task.type) throw new Error("Task type is required");
    if (!task.outputDir) throw new Error("Output directory is required");
  }
}
