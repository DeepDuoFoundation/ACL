...
export class DigitalTwinV2 {
  ...
  async calibrate(data: CalibrationData): Promise<{
  ...
  async simulate(mask: number[][], layer: string): Promise<SimulationResult> {
  ...
  private computeAerialImage(mask: number[][], calibration: CalibrationData | null): number[][] {
  ...
  private computeResistProfile(aerialImage: number[][], calibration: CalibrationData | null): number[][] {
  ...
  private computeEPE(resistProfile: number[][]): number {
    let epe = 0;
    let count = 0;
  ...
  private computeCD(resistProfile: number[][]): number {
    let cd = 0;
  ...
  private computeProcessWindow(calibration: CalibrationData | null): number {
