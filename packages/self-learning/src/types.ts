export interface TapeOutData {
  ...
  maskQuality: number;
  yieldOutcome: number;
  correctionTrajectory: Array<{ iteration: number; epe: number; reward: number }>;
  engineerOverrides: Array<{ stepId: string; originalDecision: string; overrideDecision: string }>;
  timestamp: number;
}
