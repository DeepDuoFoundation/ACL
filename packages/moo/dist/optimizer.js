...
export class MOOOptimizer {
    ...
    getTradeOffAnalysis(front) {
        const analysis = [];
        const objectives = this.config.objectives;
        for (let i = 0; i < objectives.length; i++) {
            for (let j = i + 1; j < objectives.length; j++) {
                const correlation = this.computeCorrelation(front.map((s) => s.objectives[objectives[i].name]), front.map((s) => s.objectives[objectives[j].name]));
                analysis.push({
